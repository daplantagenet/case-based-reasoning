...
**Note 1:** In the initialization step, we dropped all cases with missing values in the variables of ` data` and ` endPoint`. The user have to do missing value handling.

**Note 2:** The `data.table` returned from `coxBeta$get_similar_cases` has four additional columns:

1. `caseId`: By this column, you may map the similar cases to cases in data, e.g. if you had chosen ` k = 3`, then the first three elements in the column `caseId` will be ` 1` (following three ` 2` and so on). This means that these three cases are the three most similar cases to case ` 0` in verum data.
...
Alternatively, you may be interested in the distance matrix. Then you go this way:
