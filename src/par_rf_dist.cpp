...
// [[Rcpp::export]]
NumericMatrix getMatrixDistances(DataFrame df,  DataFrame member, int w=2) {
  ...
  void operator()(std::size_t begin, std::size_t end) {
    for (std::size_t i = begin; i < end; i++) {
      for (std::size_t j = 0; j < i; j++) {      
        ...
        mDist(i, j) = mDist(j, i) = 1. - sum / tmpTree;
      }
    }
  }
};
...
// [[Rcpp::export]]
NumericMatrix getParallelMatrixDistances(DataFrame df,  DataFrame member, int w=2){
